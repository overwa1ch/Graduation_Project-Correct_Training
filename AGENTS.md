...
## 6. 编辑器开发原则
以下原则来自产品定位，在编辑相关功能时必须遵守：
- **文档不是字符串**：禁止用 `String content` 存储复杂文档，必须采用结构化内容（块级）。
- **不允许临时方案**：禁止使用简单字符串、patch、hack 绕过结构问题。
- **编辑体验优先**：若当前实现限制了用户的编辑能力，允许且鼓励直接重构。
- **编辑器是独立系统**：应将编辑器视为可扩展、可维护、可持续迭代的独立系统，而不是普通的页面组件。