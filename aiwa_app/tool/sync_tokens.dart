...
class TokenSyncer {
  ...
  Future<Directory> _pullTokens() async {
    ...
    if (config.useLocal) {
      ...
      logger.info('从现有 lib/theme/tokens/ 拉取（MCP 需先更新源文件）');
      await _pullFromLib(tmpDir);
    }
  ...
  Future<void> _pullFromLib(Directory tmpDir) async {
    final sourceDir = Directory(libTokensPath);
    if (!sourceDir.existsSync()) {
      throw Exception('源目录不存在: $libTokensPath\n请先使用 Cursor/AI 更新 lib/theme/tokens/');
    }
  ...
  Future<Map<String, FileChange>> _calculateChanges(Directory tmpDir) async {
    ...
    for (final filename in tokenFiles) {
      ...
      final assetsFile = File(p.join(assetsTokensPath, filename));

      final tmpHash = await fileManager.calculateHash(tmpFile);
      final assetsHash =
          assetsFile.existsSync() ? await fileManager.calculateHash(assetsFile) : '';

      final tmpSize = await tmpFile.length();
      final assetsSize = assetsFile.existsSync() ? await assetsFile.length() : 0;

      final status = assetsFile.existsSync()
          ? (tmpHash == assetsHash ? ChangeStatus.unchanged : ChangeStatus.modified)
          : ChangeStatus.added;
  ...
  Future<void> _generateReport(Map<String, FileChange> changes) async {
    ...
    report.writeln('**来源**: ${config.useLocal ? "本地目录 (${config.tokensDir})" : "lib/theme/tokens/ (需先通过 MCP 更新)"}');
