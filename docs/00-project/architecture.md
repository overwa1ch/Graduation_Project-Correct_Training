...
- **aiwa_app**：面向终端用户，封装姿态引擎选择、证据降级策略、离线分析和云端 API 客户端。
...
### 3.1 离线分析流程
1. **素材准备**：`aiwa_app` 将视频与规则加载到工作空间，通过姿态引擎选择器初始化 ML Kit 或 MoveNet。
2. **姿态推理**：通过选定的姿态引擎逐帧推理生成中立关键点，统计推理耗时与质量指标。
...
## 4. 部署与运行模式
- **移动离线**：`aiwa_app` 通过 `aiwa_core` 内核实现完整的离线分析能力，无需网络即可完成姿态检测、计数与质量评估。
...
- `aiwa_app/`：Flutter 应用（UI、服务、适配器、主题、离线分析）。
